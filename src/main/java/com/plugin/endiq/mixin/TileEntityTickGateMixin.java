 ...
 * confirmed by that same mixin. isTicking(BlockPos) is private on LevelChunk,
 * so it's called through LevelChunkIsTickingAccessor's @Invoker rather than
 * directly - a plain instance.isTicking(pos) call fails javac's compile-time
 * access check even though this redirect ends up merged into the same
 * class at runtime. We don't have a confirmed real signature for a
 * nearest-player-distance API in either reference source, so this throttles
...
public abstract class TileEntityTickGateMixin {
	...
	private boolean turtlePerformer$gatedIsTicking(LevelChunk instance, BlockPos pos) {
		...
		boolean vanillaAllowsTick = ((LevelChunkIsTickingAccessor) instance).turtlePerformer$callIsTicking(pos);
